...
import { deliverPhaseMessage, phaseMessages, pendingPhaseMessages, readPhaseMessages, acknowledgePhaseMessage } from '../../../src/main/engine/phase-messages.js';
...
import { createHarness, addOrigin, buildAgent, codePhase, agentPhase, pipe, reviewEnvelope, prEnvelope, prWriter, openPrPhase, runForHarness } from './executor-harness.js';

const ruling = 'Model note was pipeline-only; screenshots belong in the PR body; the plan staying untracked is intentional; read-only waiting is correct without a desktop route.';

function fixture() {
  const h = createHarness();
  const runId = 'run_direction';
  const pipeline = pipe([agentPhase('final_review'), agentPhase('other')]);
  h.tracer.startRun({ runId, projectId: h.project.id, pipeline, request: 'review', engineer: 'test', worktreePath: h.repo, branch: null, baseRef: 'main', mode: 'pi' });
  const phaseId = h.tracer.openPhase({ runId, seq: 0, name: 'final_review', kind: 'agent', owner: 'builder', description: '' });
  const otherId = h.tracer.queuePhase({ runId, seq: 1, name: 'other', kind: 'agent', owner: 'builder', description: '' });
...
describe('durable phase direction', () => {
  it('persists without resuming, scopes acknowledgment, and advances the trace cursor at every state', () => {
    ...
    expect(() => acknowledgePhaseMessage(h.tracer, h.runId, h.phaseId, { messageId: note.messageId, status: 'acted_on', reason: 'done' })).toThrow('not been read');
    ...
    expect(h.tracer.eventsAfter(h.runId, cursor).find((event) => event.eventId === note.messageId)?.payload.direction).toMatchObject({ status: 'read' });
    expect(() => acknowledgePhaseMessage(h.tracer, h.runId, h.otherId, { messageId: note.messageId, status: 'acted_on', reason: 'wrong phase' })).toThrow();
    expect(() => acknowledgePhaseMessage(h.tracer, h.runId, h.phaseId, { messageId: note.messageId, status: 'dismissed', reason: ' ' })).toThrow();
    acknowledgePhaseMessage(h.tracer, h.runId, h.phaseId, { messageId: note.messageId, status: 'dismissed', reason: 'A separate reproducible code defect remains.' });
    expect(phaseMessages(h.tracer, h.runId)[0]).toMatchObject({ status: 'dismissed', readAt: expect.any(String), resolvedAt: expect.any(String), reason: 'A separate reproducible code defect remains.' });
  ...
  it('reads a mid-turn note on the next call and does not return the old verdict', async () => {
    ...
    let calls = 0;
    const session = new AgentSession(buildAgent(), { runId: h.runId, worktree: h.repo, tracer: h.tracer, protectedPaths: [], transport: (req) => {
      ...
      transport.send = async (text, opts) => {
        ...
        if (calls === 1) {
          ...
          acknowledgePhaseMessage(h.tracer, h.runId, h.phaseId, { messageId: note.messageId, status: 'acted_on', reason: 'Re-evaluated all four points and the code.' });
        }
  ...
  it('does not accept an ignored note or continue after an explicit interrupt', async () => {
    ...
    const session = new AgentSession(buildAgent(), { runId: h.runId, worktree: h.repo, tracer: h.tracer, protectedPaths: [], transport: (req) => {
      const transport = scripted.transport(req);
      const send = transport.send.bind(transport);
      transport.send = (text, opts) => { opts?.direction?.(); return send(text, opts); };
      return transport;
    } });
    await expect(session.send('review', { phaseId: h.phaseId })).rejects.toThrow('did not acknowledge');
    expect(phaseMessages(h.tracer, h.runId)[0]).toMatchObject({ messageId: note.messageId, status: 'read' });
    await session.close();

    const stalled = new ScriptedAgent(['unused'], [], [], { stallOnTurns: [0] });
    const live = new AgentSession(buildAgent(), { runId: h.runId, worktree: h.repo, tracer: h.tracer, protectedPaths: [], transport: (req) => stalled.transport(req) });
  ...
  it('re-evaluates a rejected reviewer in the same worktree and conversation, then reaches open_pr without rebuilding', async () => {
    const h = createHarness();
    addOrigin(h.repo);
    const reviewer = buildAgent({ name: 'reviewer', envelope: 'review', writes: [], toolProfile: 'read-only' });
    const agents = [reviewer, prWriter()];
    const pipeline = pipe([
      codePhase('screenshot_verify', { argv: ['sh', '-c', 'echo verified >> verification-count; echo keep > plan.md'] }),
      agentPhase('final_review', { agent: 'reviewer', envelope: 'review', gates: ['verdict_consistent', 'disapproval_halts'] }),
      openPrPhase(),
    ]);
    const first = await runForHarness(h, { agents, pipeline, scripted: new ScriptedAgent([JSON.stringify({ ...JSON.parse(reviewEnvelope(false)), status: 'fail' })]) });
    ...
    const deps: ExecutorDeps = {
      tracer: h.tracer, runId: first.runId, project: h.project, pipeline, agents, engineer: 'test', request: 'review', supportDir: h.support,
      envelopeDefs: [], envelopeRetries: 0, gateRetries: 0, compactionThreshold: 0.8, rewindAfterCorrections: 2, gh: { bin: gh.bin },
      transport: (req) => {
        const transport = scripted.transport(req);
        const send = transport.send.bind(transport);
        transport.send = async (text, opts) => {
          ...
          if (req.agent.name === 'reviewer') {
            ...
            acknowledgePhaseMessage(h.tracer, first.runId, req.phaseId()!, { messageId: note.messageId, status: 'acted_on', reason: 'All four operator rulings applied; no other code blocker found.' });
    ...
    expect(scripted.reopened.find((entry) => entry.agent === 'reviewer')?.existingSessionId).toBe(previousSessionId);
