      ...
      events: async (_projectId, _runId, _after): Promise<EventPage> => {
        ...
        const events: EventRow[] = [
          ...
          },
          {
            rowid: 3,
            changeId: 3,
            eventId: 'evt_web_write',
            runId: _runId,
            phaseId,
            parentId: null,
            type: 'tool_call',
            name: 'write: src/retry.test.ts',
            payload: {
              tool: 'write',
              kind: 'edit',
              args: {
            ...
            tokens: 0,
            startedAt: nowIso(-65_000),
            endedAt: nowIso(-60_000),
          },
          {
            rowid: 4,
            changeId: 4,
            eventId: 'evt_web_bash',
            runId: _runId,
            phaseId,
            parentId: null,
            type: 'tool_call',
            name: 'bash: pnpm exec vitest run src/retry.test.ts',
            payload: {
              tool: 'bash',
              kind: 'command',
              args: { command: 'pnpm exec vitest run src/retry.test.ts' },
              result:
                ' ✓ src/retry.test.ts (1)\n\n Test Files  1 passed (1)\n      Tests  1 passed (1)\n',
            },
            tokens: 0,
            startedAt: nowIso(-55_000),
            endedAt: nowIso(-45_000),
          },
          {
            rowid: 5,
            changeId: 5,
            eventId: 'evt_web_read',
            runId: _runId,
            phaseId,
            parentId: null,
            type: 'tool_call',
            name: 'read: src/retry.ts',
            payload: {
              tool: 'read',
              kind: 'read',
              args: { path: 'src/retry.ts', offset: 1, limit: 20 },
              result:
                'export async function fetchWithRetry(url: string, attempts = 3): Promise<Response> {\n  // …\n}\n',
            },
            tokens: 0,
            startedAt: nowIso(-40_000),
            endedAt: nowIso(-38_000),
          },
          {
            rowid: 6,
            changeId: 6,
            eventId: 'evt_web_grep',
            runId: _runId,
            phaseId,
            parentId: null,
            type: 'tool_call',
            ...
            payload: {
              ...
              result: 'src/retry.ts:1\nsrc/retry.test.ts:2\n',
            },
            tokens: 0,
            startedAt: nowIso(-35_000),
            endedAt: nowIso(-33_000),
          },
        ];
        return { events: _after < 6 ? events : [], cursor: 6 };
      },
