...
export default function PromptPreview({ agent, onClose }: { agent: AgentDef; onClose: () => void }): React.JSX.Element {
  const SAMPLE = { request: 'Add rate limiting to the public API', worktree: '/Users/you/repo/.foundry-worktrees/run_260806_a1b2c3', runId: 'run_260806_a1b2c3' };
  const rendered = useMemo(() => agent.userPrompt
    .replace(/\{\{\s*request\s*\}\}/g, SAMPLE.request)
    .replace(/\{\{\s*worktree\s*\}\}/g, SAMPLE.worktree)
    .replace(/\{\{\s*run_id\s*\}\}/g, SAMPLE.runId)
    .replace(/\{\{\s*([\w.]+)\s*\}\}/g, '«$1»'), [agent.userPrompt]);
      ...
      <div className="scrim" onClick={(e) => e.target === e.currentTarget && onClose()}>
        <section className="sheet card">
          ...
          <div className="scroll body">
            ...
            <p className="faint note">The declared inputs for the phase, then the exact JSON shape of the <strong>{agent.envelope}</strong> envelope, generated from the schema the reply is validated against.</p>
          </div>
        </section>
      </div>
      <style>{`
