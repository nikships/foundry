...
export const BUILTIN_AGENTS: AgentDef[] = [
  ...
  },
  {
    ...
    userPrompt: [
      ...
      'Findings are the payload; do not invent dummy artifacts.',
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      '## Purpose',
      '',
      'Record this run on the shared pipeline branch, push it, and draft a concise, human-readable pull request title and body.',
      '',
      '## Instructions',
      '',
      '- You work in the one worktree this pipeline already opened. Do not create another worktree or branch.',
      '- Inspect what this run produced (`git status`, `git_diff`, and `read` as needed). Commit every remaining change that belongs to this run. If the tree is already clean, do not invent a commit.',
      '- Write a clear commit message from the work and prior envelopes. Stage with `git add` only those paths; do not amend earlier commits.',
      '- Push the current branch to the remote (`git push -u origin HEAD` or the equivalent for this repo). Do not force-push.',
      '- Then draft the pull request. Locate a PR template by checking these paths in order; use the first file that exists:',
    ...
    userPrompt: [
      ...
      '## Task',
      '',
      "Commit remaining work on this pipeline's branch, push it, and draft the pull",
      'request. Envelopes from the phases that did the work are appended below;',
      'treat them as the record of what happened.',
      '',
      '1. Inspect `git status` and `git_diff`. Commit remaining run changes if the tree is dirty; skip the commit if it is already clean.',
      '2. Push the current branch to the remote. Do not force-push.',
      '3. Locate a PR template in the documented search order and follow it if found.',
      '4. Compose a bounded title and a non-empty markdown body.',
      '5. Report `title` and `body` in the envelope.',
    ].join('\n'),
  },
  {
    name: 'issue_writer',
