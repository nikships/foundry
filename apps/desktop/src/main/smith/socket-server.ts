/**
 * The Smith transport server. Listens on a unix domain socket whose path lands
 * in `FOUNDRY_SMITH_SOCKET`; the helper CLI (spawned inside droid) connects and
 * speaks the newline-delimited JSON protocol in `protocol.ts`.
 *
...
import { drainLines, encodeLine, type CliRequest, type CliResponse } from './protocol.js';
...
export class SmithSocketServer {
  ...
  private async write(request: CliRequest, projectId?: string): Promise<CliResponse> {
    ...
    const { name, issues, overwrites } = this.prepare(request, projectId);
    if (!name) return { ok: false, error: `${request.kind} spec is missing its name` };
