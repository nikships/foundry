...
export const SMITH_CHAT_HARNESS = [
  ...
  '- Beyond designs, present an `engineer_checkpoint` when a run is parked on',
  '  a checkpoint (the card carries the question, run/phase context, and a',
  '  draft answer, but answering it still goes through `smith_interrupts`',
  '  and its approval card); a `readiness_journey` when readiness is the',
