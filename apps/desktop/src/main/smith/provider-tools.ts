...
export function smithProvidersTool(deps: SmithActionToolDeps): ToolDefinition {
  return defineTool({
    ...
    description:
      'Read connection state without approval: state, stored_keys (presence only), linear_state, tavily_state. Approval: connect/disconnect/cancel_login(provider), set_api_key/clear_api_key(providerId), linear_test/linear_set_api_key/linear_clear_api_key, tavily_install/tavily_remove/tavily_set_api_key/tavily_clear_api_key. Read state before changes. OAuth uses provider; key operations use providerId. API key values are entered only in the masked approval card, never in arguments or chat. A started login is not a completed connection; inspect state to confirm.',
    parameters: {
      type: 'object',
      properties: {
