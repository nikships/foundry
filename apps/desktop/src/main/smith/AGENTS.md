...
npx vitest run apps/desktop/tests/main/smith/smith-socket.test.ts      # dispatch() without a real socket
npx vitest run apps/desktop/tests/main/smith/smith-cli-args.test.ts    # pure argv + socket-path resolution
npx vitest run apps/desktop/tests/main/smith/smith-skill.test.ts       # skill/template drift guard
npx vitest run apps/desktop/tests/main/smith/smith-launch.test.ts      # shell quoting + asar path rewriting
```
