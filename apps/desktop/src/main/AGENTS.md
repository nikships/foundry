...
- **A generated plan appoints every agent phase's model explicitly.** Automatic casting uses the union of explicit Settings → Agent Defaults and per-run Orchestrator model pins; if both inherit, it uses the full enabled catalog (`pi/enabled-models.ts` — reachable minus `hiddenModelIds`). A configured pin that is unavailable or hidden fails planning instead of broadening the pool. `orchestrator/plan.ts:phaseModelIssues` rejects a phase that omits `model`, writes `inherit`, or names a model outside that planning pool. `startRun` checks the live enabled catalog again, deliberately allowing the operator to re-cast a phase on the plan card before confirming, and fails closed if that catalog is unavailable. A mid-run amendment may only re-cast onto a model the confirmed plan already names. An empty catalog stands down the planning rail only when neither configuration pin constrains casting.
- **The cast pool is described by capability, not by price or context.** Each line carries the model's efforts and its Artificial Analysis Intelligence Index (`shared/model-intelligence.ts`, vendored by `npm run fetch:intelligence`). Per-token cost and context window are deliberately withheld: every pool model holds a phase prompt, and a price the operator already pays invites the model to weigh a budget it cannot see. An id the index does not name renders `intelligence unrated` — stated rather than omitted, so a missing score reads as unmeasured instead of as a formatting gap the model fills with a guessed rank.
