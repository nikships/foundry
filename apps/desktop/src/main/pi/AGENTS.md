...
- `tools.ts` — Foundry's own tools (`report_progress`, `read_phase_context`, `submit_envelope`, which replaced the MCP server), the two built-in lists (`BUILTIN_TOOLS` and the `READ_ONLY_TOOLS` a read-only session is confined to), and `runToolsFor(profile)`, the whole allowlist a run session opens with for an agent's `toolProfile`.
