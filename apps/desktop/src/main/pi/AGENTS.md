...
- `tool-names.ts` — the allowlist strings (`FOUNDRY_TOOL_NAMES`, `ONESHOT_OUTPUT_TOOL_NAME`, `BUILTIN_TOOLS`, `READ_ONLY_TOOLS`, `runToolsFor`) with no vendor import, so `session.ts` and `AppContext` can name tools without parsing the runtime.
- `tools.ts` — Foundry's run tools (`report_progress`, `read_phase_context`, `git_diff`, `submit_envelope`) plus the schema-bound one-shot answer channel `submit_result`. `git_diff` is the read-only diff affordance: it runs `git diff` against the engine-supplied branch point and nothing else, takes no ref and no argv, and bounds its answer at `GIT_DIFF_MAX_CHARS` with a marker naming the files it dropped.
...
- `policy-extension.ts` — the inline pi extension. `foundryExtension()` registers Foundry's run tools, the `tool_call` hook, and `before_agent_start` (the roster role); `policyOnlyExtension()` installs the hook pair and optionally registers a one-shot's `submit_result`.
...
- **The tool list is the allowlist.** A tool absent from `createAgentSession({tools})` is absent from the registry, which is why Foundry's four run tools are named alongside the seven built-ins and `submit_result` is added only for a one-shot whose caller supplied an output schema.
...
- **A one-shot owns its session for exactly one turn.** Dispose in a `finally`, so a failed or cancelled turn releases the model connection too. When a caller requests structured output, `submit_result` is schema-bound and its captured object is only a candidate: caller-owned Zod validation and domain rails remain authoritative.
