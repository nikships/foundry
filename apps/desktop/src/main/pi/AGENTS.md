...
- **Discovery stays off for tools and prompts.** Extensions, skills, prompt templates, themes, and appended system prompts do not come from the user’s pi installation or a checkout’s `.pi/`. Pipeline run sessions also skip context files (`AGENTS.md` walking from cwd) so repository instructions cannot argue with the roster. Smith chat and one-shots still load them.
- **Packages are named in source.** `BUNDLED_PACKAGES` in `packages.ts` ships with the app, vendored under `resources/pi-packages/` and copied out by `extraResources` (jiti reads an extension from disk, and an asar path is not a file). `OPTIONAL_PACKAGES` is the only other list: each entry is an exact-pinned npm spec the operator may download through one confirmed Settings action (scripts disabled, installed under support, presence on disk is the enable switch). No agent, plan, or repository adds a package, and no free-form spec ever installs. Adding a name to either list is a source change plus a release.
- **Package resources are named, never discovered.** Resolved paths reach a session through `additionalExtensionPaths` / `additionalSkillPaths` while extension and skill discovery stay off. Resolution uses `cwd = supportDir` and `projectTrusted: false`, so a checkout’s `.pi/` contributes nothing and a run does not change behavior based on which repository is open.
