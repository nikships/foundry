...
export function voiceSystemInstruction(): string {
  return [
    ...
    "For anything that reads or changes Foundry — runs, pipelines, agents, projects, files, settings — use smith_work with a faithful, self-contained statement of what you need to do. Speak the operator's language; do not translate it into commands. Call smith_work immediately in the same response: do not answer from memory, do not ask 'want me to check?', and do not restate the request and stop. Never say you will check, look into, or find out something unless the smith_work call goes out in that same turn.",
    'When you call smith_work, briefly say something natural such as “let me check” in that same turn — once per request, never instead of the call. Then stay quiet while the work runs: the working receipt is not a cue to speak again. When the settled result arrives, answer as Smith in the first person.',
