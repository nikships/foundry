...
- **Compaction occurs only between phases.** Foundry supplies the summary
  (request, current phase, artifacts, open failures, envelope fields, and
  any files Pi recorded as edited) and pins the phase prompt. Pi
  auto-compaction stays off.
...
- **Pipeline healing is Smith's final recovery layer, distinct from command healing.** When a run would otherwise terminate unsuccessfully, Smith proposes a validated tail replacement starting at the failure point and the same run continues in the same worktree; the engine validates and applies it with no approval detour. It covers manual and generated phase failures and final acceptance rejections with a phase anchor. Two Smith **call** slots per run ID survive Continue; a started call counts even when cancelled. Empty proposals leave the pipeline unchanged, and accepted, cancelled, or interrupted outcomes never trigger healing. Command healing keeps its own per-phase repair turns and neither consumes nor replenishes the global Smith budget.
