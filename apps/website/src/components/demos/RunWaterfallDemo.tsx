 ...
 *
 * Replays a real Refine → Build → Ship → PR trace: phase lanes fill left to
 * right on a shared timeline, each bar segmented into one tick per tool call
 * and coloured by what kind of call it was. The right pane streams that phase's
 * transcript and switches to whichever phase is running, unless you pin one by
 * clicking its lane.
 *
 * The per-phase header carries what the app's header carries: the model that
 * actually answered, the tokens spent, and how much of that model's context the
 * phase occupied.
 *
 * Mirrors apps/desktop/src/renderer/components/run/Waterfall.tsx and
 * apps/desktop/src/renderer/components/inspector/TranscriptLane.tsx.
 */

import { useEffect, useMemo, useRef, useState } from 'react';
import { RUN, TOOL_COLOR, KIND_LABEL, agentColor, CHECK_DESCRIPTIONS } from '../../data/foundry';
import type { RunPhase, ToolCall } from '../../data/foundry';
import { useInView, useRafElapsed, useReducedMotion } from '../../hooks';
import { Badge, Dot } from '../ui';

/** Wall-clock seconds it takes to replay the whole run. */
const REPLAY_SECONDS = 10;
const HOLD_SECONDS = 2.4;

const pad = (n: number) => String(Math.floor(n)).padStart(2, '0');
const fmtDur = (s: number) =>
  s >= 60 ? `${Math.floor(s / 60)}m ${pad(s % 60)}s` : `${Math.round(s)}s`;
const fmtClock = (s: number) => `13:${pad(43 + Math.floor(s / 60))}:${pad(Math.floor(s) % 60)}`;
...
const LEGEND: Array<keyof typeof TOOL_COLOR> = [
  'read',
  'edit',
  'bash',
  'check',
...
type Tab = 'timeline' | 'report' | 'checks';
...
export function RunWaterfallDemo() {
  ...
  return (
    <div
      ref={ref}
      className="grid min-h-[500px] grid-cols-1 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]"
    >
      ...
      <div className="flex flex-col border-b border-line lg:border-b-0 lg:border-r">
        <header className="border-b border-line px-5 pb-4 pt-[18px]">
          ...
          <p className="mt-[10px] max-w-[60ch] text-[13.5px] leading-[1.55] text-text-dim">
            {RUN.request}
          </p>
        ...
        <div className="flex-1 px-5 pb-5 pt-[14px]">
          <div className="flex justify-between pb-2 pl-[126px] font-mono text-[9px] tracking-[0.1em] text-text-ghost">
            {[0, 1, 2, 3, 4].map((i) => (
          ...
          {RUN.phases.map((p, i) => {
            ...
            return (
              <button
                ...
                onClick={() => setPinned(i)}
                className={`grid h-[32px] w-full grid-cols-[126px_minmax(0,1fr)] items-center gap-[10px] rounded-sm text-left transition-colors duration-fast ease-mech ${
        ...
        <div
          className={`mx-5 mb-[18px] flex flex-wrap items-center gap-x-3 gap-y-1 rounded-sm border border-line border-l-2 border-l-green bg-bg-panel px-[14px] py-3 transition-[opacity,transform] duration-slow ease-mech ${
            done ? 'translate-y-0 opacity-100' : 'translate-y-1 opacity-0'
          }`}
        >
          <span className="label-sm flex-none text-green">accepted</span>
          <span className="text-[13.5px] text-text-dim">
            <b className="font-medium text-text">open_pr</b> reported success — PR #{RUN.pr} exists
          </span>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-col bg-bg-panel">
        ...
        <div className="flex gap-[2px] border-b border-line px-3">
          {(['timeline', 'report', 'checks'] as Tab[]).map((t) => {
            const count =
              t === 'timeline'
                ? shown.trace.length
                : t === 'checks'
        ...
        {tab === 'timeline' ? (
          <div
            ref={streamRef}
            role="log"
            className="scroll max-h-[330px] min-h-[262px] flex-1 py-2 font-mono text-[11.5px] tabular-nums"
          >
            {rows.map((t, k) => (
              <TraceRow
                key={`${shown.name}-${k}`}
                call={t}
                at={shown.start + (shown.dur * (k + 1)) / shown.trace.length}
              />
            ))}
          </div>
        ) : (
          <JsonPane value={tab === 'report' ? reportFor(shown) : checksFor(shown)} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="border-b border-line px-[18px] py-4">
      <div className="flex flex-wrap items-center gap-[10px]">
        ...
        {phase.feedbackTo ? (
          <Badge color="var(--amber)">feedback → {phase.feedbackTo}</Badge>
        ) : null}
      </div>
      ...
      {phase.context !== undefined ? (
        <div className="mt-[10px] flex items-center gap-[10px]">
          <span className="font-mono text-[9px] uppercase tracking-label text-text-ghost">
            context
          </span>
          <span className="relative h-[4px] w-[92px] flex-none overflow-hidden rounded-full bg-bg-raised">
            <span
              className="absolute inset-y-0 left-0 rounded-full bg-accent"
              style={{ width: `${phase.context}%` }}
            />
          </span>
...
function TraceRow({ call, at }: { call: ToolCall; at: number }) {
  ...
  const bare = call.tool === 'assistant' || call.tool === 'report' || call.tool === 'check';
  return (
...
function JsonPane({ value }: { value: unknown }) {
  ...
  return (
    <pre className="scroll max-h-[330px] min-h-[262px] px-[18px] py-[14px] font-mono text-[11.5px] leading-[1.7] text-text-dim">
      {src.split('\n').map((line, i) => (
...
function JsonLine({ line }: { line: string }) {
  ...
  return (
    <div>
      ...
      <span className={isString ? 'text-green' : isNumber ? 'text-purple' : 'text-text-dim'}>
        {rest}
      </span>
    </div>
  );
...
function reportFor(p: RunPhase): Record<string, unknown> {
  if (p.kind === 'code') {
    return {
      phase: p.name,
      kind: 'command',
      command: p.command,
      exit_code: 0,
      duration_ms: p.dur * 1000,
      ...(p.feedbackTo ? { feedback_to: p.feedbackTo, feedback_retries: 2 } : {}),
    };
  }
  const base: Record<string, unknown> = { status: 'ok', phase: p.name, report: p.report };
  switch (p.report) {
    case 'brief':
      return {
        ...base,
    ...
    case 'build':
      return {
        ...base,
        summary: 'Token-bucket limiter keyed by API key, wired into every public route.',
...
function checksFor(p: RunPhase): unknown {
  if (!p.checks.length) {
    return { checks: [], note: 'This phase declares no checks.' };
  }
  return p.checks.map((c) => ({ check: c, passed: true, note: CHECK_DESCRIPTIONS[c] }));
}
