...
- `verify` runs from the repo root (Node 22, `npm ci`): typecheck, lint, `check:files`, `check:duplicate`, format:check, `check:docs`, knip, `test:coverage`, build, `audit:deps` (high/critical only). It does **not** run `check:css` — that gate is local-only. It does **not** run `test:e2e`.
...
npm run typecheck && npm run lint && npm run check:files && npm run check:duplicate && npm run format:check && npm run check:docs && npm run knip && npm run test:coverage && npm run build && npm run audit:deps
