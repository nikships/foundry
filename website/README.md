...
enforced in six places, all of them pointing at `website/`:

| Where | What it does |
| --- | --- |
| `tsconfig.json` (root) | `include` is `src`, `scripts`, `tests` only — root `tsc` never sees this folder |
| `eslint.config.js` | `website/**` in `ignores` |
| `.prettierignore` | `website` — so `prettier --check .` skips it |
| `knip.json` | `website/**` in `ignore` |
| `.github/workflows/ci.yml` | the push-to-`main` `paths` allowlist does not include `website/**`, so the `verify` job does not run for site-only pushes |
| `.github/codeql/codeql-config.yml` | `website` in `paths-ignore` — the required check still reports, it just does not scan this folder |
...
| --- | --- |
| `AGENTS` | `src/main/store/builtin-agents.ts` |
| `PIPELINES` | `src/main/store/builtin-pipelines.ts` |
| `GATE_DESCRIPTIONS` | `src/main/engine/gates.ts` |
| `ENVELOPE_BLURBS` | `src/main/store/envelopes.ts` |
| design tokens | `src/renderer/design/tokens-base.css`, `tokens-factory.css` |
