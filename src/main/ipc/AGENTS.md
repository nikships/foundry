...
Keep routers domain‑scoped; update the shared contract before wiring a new channel. `tests/ipc-clone.test.ts` and `tests/ipc-surface.test.ts` guard the surface.
...
Exactly eight push channels (from main → renderer):
